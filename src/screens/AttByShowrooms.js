...
const AttByShowrooms = ({ navigation }) => {
  ...
  const [attAllBy, setAttAllBy] = useState('');
  ...
  const searchAttByEmp = async () => {
    if (selectDate == "ថ្ងៃចាប់ផ្តើម" && selectDateEnd == "បញ្ចប់" && employees == "") {
      ...
      setLoading(true);
      await request("getAttByShowrooms.php", "POST", param).then(res => {
        setLoading(false);
        if (res) {
          setAttAllBy(res);
        } else {
  ...
  return (
    <Layout loading={loading}>
      <View style={styles.headerContainer}>
        ...
        />
        <MainText title='វត្តបុគ្គលិកតាមសាខា' type='default' />
      </View>
      ...
      {/* <View style={styles.empContainer}>
        <MainText title={profile?.project_name} />
        <View style={{ justifyContent: 'center', alignSelf: 'center', alignItems: 'center' }}>
          <FastImage source={Images.LOGO} resizeMode='contain' style={{ width: 200, height: 200, opacity: 0.1 }} />
