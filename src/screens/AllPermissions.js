...
const AllPermissions = ({ navigation }) => {
  ...
  const getAllPermissions = async () => {
    if (selectDate == 'ថ្ងៃចាប់ផ្តើម' && selectDateEnd == 'បញ្ចប់' && employees == "") {
      setLoading(true);
      await request("getAllPermisions.php", "POST", {}).then(res => {
        setLoading(false);
        if (res) {
      ...
      setLoading(true);
      await request("getAllPermisions.php", "POST", param)
        .then(res => {
          setLoading(false);
          if (res) {
            setAllPermissions(res);
  ...
  return (
    <Layout loading={loading}>
      <View style={styles.headerContainer}>
        ...
        />
        <MainText title='ច្បាប់បុគ្គលិកទាំងអស់' type='default' />
      </View>
